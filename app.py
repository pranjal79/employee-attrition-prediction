...
.metric-card p  { font-size: 0.85rem; color: #6c757d; margin: 4px 0 0 0;
...
.risk-badge { display: inline-block; padding: 6px 20px; border-radius: 50px;
              font-size: 1rem; font-weight: 700; letter-spacing: 0.05em; margin-top: 6px; }
...
if missing:
    ...
    st.stop()   # Halt app — nothing below runs if files are missing
...
with st.sidebar:
    ...
    marital_status = st.selectbox("Marital Status", ["Divorced", "Married", "Single"], index=1)
    ...
    department = st.selectbox(
        "Department",
        ["Human Resources", "Research & Development", "Sales"], index=1
    )
...
# =============================================================================
# STEP 7: Build input DataFrame in exact feature order
# =============================================================================
...
# Reorder to exactly match model training feature order
...
if attrition_pct < 35:
    risk_level = "Low";    badge_class = "badge-low";    card_class = "card-green"; risk_emoji = "🟢"
elif attrition_pct < 60:
    risk_level = "Medium"; badge_class = "badge-medium"; card_class = "card-orange"; risk_emoji = "🟠"
else:
    risk_level = "High";   badge_class = "badge-high";   card_class = "card-red";   risk_emoji = "🔴"
...
with st.expander("📋 View Current Employee Input Data", expanded=False):
    st.dataframe(
        pd.DataFrame({"Feature": feature_names, "Value": input_data.values[0]}),
        use_container_width=True, hide_index=True
    )

# ── SHAP Waterfall ────────────────────────────────────────────────────────────
st.markdown('<div class="section-header">🔍 Why Is This Employee At Risk? (SHAP Waterfall)</div>', unsafe_allow_html=True)
st.markdown("Each bar shows how much a feature **increases** 🔴 or **decreases** 🔵 the attrition risk.")
...
with tab1:
    if os.path.exists(BAR_PATH):
        st.image(BAR_PATH, caption="Mean |SHAP Value| per feature", use_container_width=True)
    else:
